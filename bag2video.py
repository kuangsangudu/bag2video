...
class Bag2video:
        ...
        try:
            ...
            while True:
                ...
                if data_stream == rs.stream.color:
                    ...
                    data_frame_image = np.array(depth_frame.get_data())
                else:
    ...
    def bag2image(self, path, data_stream=rs.stream.color, data_format=rs.format.rgb8, fps=30):
        self._bag2trans(path, data_stream, data_format, fps)
    ...
    def bag2video(self, path, data_stream=rs.stream.color, data_format=rs.format.rgb8, fps=30, size=(1280, 720)):
        ...
        iscolor = True if data_stream == rs.stream.color else False
        if file_extension == "mp4":
            video = cv2.VideoWriter(path, cv2.VideoWriter_fourcc('M', 'P', '4', 'V'), fps, size, iscolor)
        elif file_extension == "avi":
            video = cv2.VideoWriter(path, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, size, iscolor)
        else:
            logger.warning("I haven't decided on this part")
            return
        self._bag2trans(path, data_stream, data_format, fps, 1, video)
